...
%% Robô de movimento retilíneo-determinístico
% Matheus Cabral da Silva
% 2021

%[RotationAngle(graus) iniX(m) iniY(m) iniAngle(rad) Rounds]
conditions = [
              90 0.3 0.3 0 1;
              95 0.3 0.3 0 1;
              100 0.3 0.3 0 1;
...
for conditionLoop = startFrom:numConditions 
    for roundLoop = startRound:conditions(conditionLoop,5)
        %% Criando ambiente
        load('sala_comum.mat')
        %load('sala_vazia.mat')
        ...
        initialAngle = conditions(conditionLoop,4);               % Ângulo inicial [rad]
        ...
        for idx = 2:numel(tVec)
            ...
            if endOfRotation == true
                pose(:,idx) =  [pose([1,2],idx-1); presentAngle];
            else
