...
%% Robô de movimento duplamente estocástico
% Matheus Cabral da Silva
% 2021

% [minAngle(graus) maxAngle(graus) iniX(m) iniY(m) iniAngle(rad) timeRotation Rounds]
conditions = [
                0 120 0.3 0.3 0 0.5 10;
                0 120 0.3 0.3 0 1 10;
                0 120 0.3 0.3 0 2 10;
                0 120 0.3 0.3 0 3 10;
                0 120 0.3 0.3 0 4 10;
             ];

         
startFrom = 1;
startRound = 1;
folder = 'com_teste_dupl_est_final';

sizeConditions = size(conditions);
numConditions = sizeConditions(1,1);

for conditionLoop = startFrom:numConditions 
    for roundLoop = startRound:conditions(conditionLoop,7)
        %% Criando ambiente
        load('sala_comum.mat')
        %load('sala_vazia.mat')
        ...
        GeneralMovementRotationTime = conditions(conditionLoop,6); % Tempo pelo qual o robô executará rotação em movimento geral [s]

        viz.robotRadius = 0.15; 
        ...
        for idx = 2:numel(tVec)
            ...
            if cooldown <= 0
                if any(ranges <= viz.robotRadius + extraSpace)
                    ...
                    vRef = MaxV;
                    if cooldownGeneralMovement <=0
                        wRef = (-MaxW + (MaxW-(-MaxW))*rand);
                        cooldownGeneralMovement = GeneralMovementRotationTime;
                    else
                        cooldownGeneralMovement = cooldownGeneralMovement - sampleTime;
                    end
                end
            else
