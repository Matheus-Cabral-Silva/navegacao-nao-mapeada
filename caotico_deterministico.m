...
%% Robô de movimento caótico-determinístico
% Matheus Cabral da Silva
% 2021

%[RotationAngle(graus) iniX(m) iniY(m) iniAngle(rad) vE1 vE2 vE3 vE4 vE5 A B C Rounds]
conditions = [
              120 0.3 0.3 0 4 3.5 0 1 1 0.5 0.25 0.25 1;
              120 0.3 0.3 0 5 3.5 0 1 1 0.5 0.25 0.25 1;
              120 0.3 0.3 0 4 4.5 0 1 1 0.5 0.25 0.25 1;
              120 0.3 0.3 0 4 3.5 1 1 1 0.5 0.25 0.25 1;
              120 0.3 0.3 0 4 3.5 0 1 1 1.5 0.25 0.25 1;
              120 0.3 0.3 0 4 3.5 0 1 1 0.5 1.25 0.25 1;
              120 0.3 0.3 0 4 3.5 0 1 1 0.5 0.25 1.25 1;
              120 0.3 0.3 0 3 3.5 0 1 1 0.5 0.25 0.25 1;
              120 0.3 0.3 0 4 2.5 0 1 1 0.5 0.25 0.25 1;
              120 0.3 0.3 0 4 3.5 -1 1 1 0.5 0.25 0.25 1;
              120 0.3 0.3 0 4 3.5 0 1 1 -0.5 0.25 0.25 1;
              120 0.3 0.3 0 4 3.5 0 1 1 0.5 -0.75 0.25 1;
              120 0.3 0.3 0 4 3.5 0 1 1 0.5 0.25 -0.75 1;
             ];
startFrom = 1;
startRound = 1;
folder = 'comum_teste_caot_det';

sizeConditions = size(conditions);
numConditions = sizeConditions(1,1);

for conditionLoop = startFrom:numConditions 
    for roundLoop = startRound:conditions(conditionLoop,13)
        %% Criando ambiente
        load('sala_comum.mat')
        %load('sala_vazia.mat')
        ...
        RotationAngle = conditions(conditionLoop,1);        % Ângulo em caso de colisão [Degrees]
        ...
        initialAngle = conditions(conditionLoop,4); % Ângulo inicial
        presentAngle = initialAngle;    % Ângulo para retificação - modelo ideal

        tVec = 0:sampleTime:simulationTime;   
        initPose = [conditions(conditionLoop,2); conditions(conditionLoop,3); initialAngle]; % Posição inicial (x y theta)
        pose = zeros(3,numel(tVec));    % Matriz de posição
        pose(:,1) = initPose;

        %% Constantes para movimento caótico
        stateVariables=[conditions(conditionLoop,5) conditions(conditionLoop,6) conditions(conditionLoop,7) conditions(conditionLoop,8) conditions(conditionLoop,9)];
        constants  = [conditions(conditionLoop,10) conditions(conditionLoop,11) conditions(conditionLoop,12) MaxV]; %Constantes A, B e C e v (velocidade linear)
        ...
        for idx = 2:numel(tVec)
            ...
            if cooldown <= 0
                if any(ranges <= viz.robotRadius + extraSpace)
                    vRef = 0;
                    angleIncrementDegrees = RotationAngle;
                ...
                if cooldown < 0
                    if clockwiseRotation == true
                        presentAngle = presentAngle - RotationAngle*pi/180;
                    else
                        presentAngle = presentAngle + RotationAngle*pi/180;
                    end

                    if presentAngle > 2*pi
                        presentAngle = presentAngle - 2*pi;
                    end
